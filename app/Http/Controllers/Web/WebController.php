...
class WebController extends Controller
{
    ...
    public function posts(Request $request)
    {
        ...
        // Lấy danh sách các admin có status là 1 và module access chứa "posts_management", hoặc có name là "Master Admin"
        $adminsWithPostsManagementAccess = Admin::where('status', 1)
            ->where(function ($query) {
                $query->whereHas('role', function ($query) {
                    $query->where('name', 'Master Admin');
                })
                    ->orWhereHas('role', function ($query) {
                        $query->whereJsonContains('module_access', 'posts_management');
                    });
            })
            ->get();
        ...
        // Áp dụng điều kiện tìm kiếm theo thẻ tag
        ...
        // Lấy ra bài viết với status "published"
        $posts = $query->where('status', 'published')->paginate(8);

        // Lấy ra bài viết sự kiện mới nhất
        ...
        if ($request->ajax()) {
            return response()->json([
                'view' => view('web-views.posts._ajax-posts', compact('posts'))->render(),
            ], 200);
        }

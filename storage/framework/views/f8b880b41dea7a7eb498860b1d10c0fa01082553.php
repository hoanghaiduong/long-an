...
<head>
    ...
    <!-- Favicon -->
    <link rel="shortcut icon" href="<?php echo e(asset('storage/app/public/company/'.$web_config['fav_icon']->value)); ?>">
    ...
    <style>
        select {
            background-image: url('<?php echo e(asset('/public/assets/back-end/img/arrow-down.png')); ?>');
            background-size: 7px;
...
<script>
    <?php if(\App\CPU\Helpers::module_permission_check('order_management') && env('APP_MODE')!='dev'): ?>
    setInterval(function () {
        $.get({
            url: '<?php echo e(route('admin.get-order-data')); ?>',
            dataType: 'json',
...
<script>
    function call_demo() {
        toastr.info('<?php echo e(translate("update_option_is_disabled_for_demo")); ?>!', {
            CloseButton: true,
            ProgressBar: true
        });
    }
...
<script>
    function toogleModal(e, toggle_id, on_image, off_image, on_title, off_title, on_message, off_message) {
        ...
        if ($('#'+toggle_id).is(':checked')) {
            ...
            $('#toggle-image').attr('src', "<?php echo e(asset('/public/assets/back-end/img/modal')); ?>/"+on_image);
            ...
            $('#toggle-image').attr('src', "<?php echo e(asset('/public/assets/back-end/img/modal')); ?>/"+off_image);
            $('#toggle-ok-button').attr('toggle-ok-button', toggle_id);
        }
    ...
    function toogleStatusModal(e, toggle_id, on_image, off_image, on_title, off_title, on_message, off_message) {
        ...
        if ($('#'+toggle_id).is(':checked')) {
            ...
            $('#toggle-status-image').attr('src', "<?php echo e(asset('/public/assets/back-end/img/modal')); ?>/"+on_image);
            ...
            $('#toggle-status-image').attr('src', "<?php echo e(asset('/public/assets/back-end/img/modal')); ?>/"+off_image);
            $('#toggle-status-ok-button').attr('toggle-ok-button', toggle_id);
        }
...
<?php /**PATH C:\xampp\htdocs\long-an\resources\views/layouts/back-end/app.blade.php ENDPATH**/ ?>
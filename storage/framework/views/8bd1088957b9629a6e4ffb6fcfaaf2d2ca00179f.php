...
<head>
    ...
    <style>
        ...
        span.badge-accent {
            color: var(--base);
            background-color: <?php echo e($web_config['primary_color']); ?>40;
        }
    ...
    <?php if($pixel_analytices_user_code): ?>
        <!-- Facebook Pixel Code -->
            ...
            <img height="1" width="1" style="display:none"
                src="https://www.facebook.com/tr?id=<?php echo e($pixel_analytices_user_code); ?>&ev=PageView&noscript=1"/>
            </noscript>
...
<a class="btn-scroll-top btn--primary" href="#top" data-scroll>
    <span class="btn-scroll-top-tooltip text-muted font-size-sm mr-2"><?php echo e(translate('top')); ?></span><i
...
<script src="<?php echo e(asset('public/assets/front-end')); ?>/js/custom.js"></script>
...
<script>
            ...
            success: function (data) {
                ...
                if (parseInt($('.countWishlist').html()) % 15 === 0) {
                    if ($('#wishlist_paginated_page').val() == 1) {
                        ...
                        window.location.href = '<?php echo e(route('wishlists', ['page' => ''])); ?>' + (page_value - 1);
                    }
                }
            },
            complete: function () {
    ...
    function addToCart(form_id = 'add-to-cart-form', redirect_to_checkout=false) {
        if (checkAddToCartValidity()) {
            ...
            Swal.fire({
                type: 'info',
                title: 'Cart',
                text: '<?php echo e(translate("please_choose_all_the_options")); ?>'
            });
        }
    }
    ...
    function currency_change(currency_code) {
        ...
        $.ajax({
            ...
            success: function (data) {
                toastr.success('<?php echo e(translate("currency_changed_to")); ?>' + data.name);
                location.reload();
            }
        });
    }
            ...
            $('.input-number').change(function () {
                ...
                if (valueCurrent >= minValue) {
                    ...
                    Swal.fire({
                            icon: 'error',
                            title: 'Cart',
                            text: '<?php echo e(translate("sorry_the_minimum_order_quantity_does_not_match")); ?>'
                    });
                ...
                if (productType === 'digital' || valueCurrent <= maxValue) {
                    ...
                    Swal.fire({
                            icon: 'error',
                            title: 'Cart',
                            text: '<?php echo e(translate("sorry_stock_limit_exceeded.")); ?>'
                    });
                    $(this).val($(this).data('oldValue'));
                }
            ...
            <div class="container">
                <div class="d-flex flex-wrap align-items-center justify-content-between column-gap-4 row-gap-3">
                    ...
                    <div class="btn-wrapper">
                        <span class="text-white cursor-pointer" id="cookie-reject"><?php echo e(translate("no_thanks")); ?></span>
                        <button class="btn btn-success cookie-accept" id="cookie-accept"><?php echo e(translate('yes_i_Accept')); ?></button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        if (referral_code_parameter) {
            <?php if(Request::is('customer/auth/sign-up*')): ?>
                if ($('#referral_code').length) {
                    $('#referral_code').val(referral_code_parameter);
                }
            <?php else: ?>
                window.location.href = "<?php echo e(route('customer.auth.sign-up')); ?>?referral_code=" + referral_code_parameter;
            <?php endif; ?>
        }
    });
</script>
...
<?php /**PATH C:\xampp\htdocs\long-an\resources\themes\default/layouts/front-end/app.blade.php ENDPATH**/ ?>